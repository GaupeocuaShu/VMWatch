...
const CreateWatch = () => {
    const { id } = useParams();
    // const
    const [watchFound, setWatchFound] = useState(true);
    ...
    useEffect(() => {
        ...
        const fetchWatchByID = async (id) => {
            await axiosClient
                .get(`/api/watches/${id}`)
                .then(({ data }) => {
                    const rawWatch = data.data;
                    console.log(rawWatch);
                    setWatch(rawWatch);
                })
                .catch(({ response }) => setWatchFound(false));
        };
        fetchData();
        fetchWatchByID(id);
    }, []);
